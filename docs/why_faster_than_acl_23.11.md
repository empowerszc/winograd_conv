...
> ⚠️ **已过时（2026-09-01 更新）**：本文的「8/9 case」是 OpenBLAS 后端 + 9 case 微基准
> + benchdnn wino_acl 口径的历史数据。最新对照用 arm_gemm 后端 + 59 形状，见
> `docs/onednn_comparison.md` §五（已闭环）：**我们的 F(4,4) 全面碾压 oneDNN 的
> wino:acl 1.15-9.5x**（同算法 PK，e2e_wino + benchdnn_wino 两列互相验证）。
> 本文保留作历史分析参照——§9 的优化历程转折点、§10 的 benchdnn vs e2e 口径差异
> 分析仍有参考价值。
>
> **日期**：2026-08-11
> **结论前置**：这个「8/9 case 快于 oneDNN」是 **t16、单次小形状调用** 微基准下的结果。它**不是**「我们的内核比 ACL 强」的证据，而是「**在固定开销主导的小负载下，我们的工程实现更契合 920F**」的证据。（2026-08-20 起适用范围收紧：NCHW 端到端、9T 线程档只赢 2/9，见 §10。）
>
...
- [10. 9 线程 NCHW 端到端实测：差距归因与路径（2026-08-20）](#10-9-线程-nchw-端到端实测差距归因与路径2026-08-20)
...
`--timing` 拆解（`timing_breakdown_920f.md` §3.4）给了这个亏吃多大：小图深通道 shape 里 Weight transform 占 S 的 **30-54%**（r15 `1,512,7,7,512` = 54.7%；r20 `1,2048,7,7,512` = 49.0%；r13 `1,256,14,14,256` = 49.5%），大图时摊薄到 ~1%（r58 `4,384,160,160,384` = 0.7%）。即「对手缓存 V、我们每次重算」最吃亏的恰好是 tiles 小的深通道 case——而我们（按 §3.1 历史 9 case）仍在这些 case 上大比分赢。这进一步坐实：**赢不在权重路径，赢在固定开销/调度/内存策略**；给 V 加跨调用缓存是最便宜的提速（详见 `timing_breakdown_920f.md` §7）。
...
`--timing` 拆解（`timing_breakdown_920f.md` §3.3）量化了「默认 naive/单线程 OpenBLAS」这栏的缺口：小 GEMM（4-tile，r16 `4,512,7,7,512`）吞吐只有 **~10 GFLOPS**，约为大 GEMM（140 GFLOPS，r0 `4,192,40,40,192`）的 **7%**。这正是接入 arm_gemm JIT 内核后预期收益最大的地方——GEMM 占比最高的小图深通道 case（GEMM 占 S 最高 74%，r16）也是它最快的瓶颈。
...
## 10. 9 线程 NCHW 端到端实测：差距归因与路径（2026-08-20）

> 前置修正：本文 §2/§9 的「8/9」是 **NHWC、16 线程、微基准** 下的结果。2026-08-20 换到 **NCHW 端到端、numactl 绑核、6 线程档（4/8/9/16/32/38）** 实测后，结论必须修正：**9 线程只赢 2/9，4-16 线程多数落后，32/38 线程赢的 3 个还依赖 oneDNN 自身高线程退化**。原始数据表与胜率分布见 `AGENTS.md`「性能优化记录」→「9 线程 NCHW 端到端实测」。

### 10.1 读数的第一课：加速比方向

加速比 = **oneDNN / 我们**，>1 才是我们快。最初误读为 <1 快，方向反了会得出完全相反的结论——「9T 只有 row 4/9 赢」一度被误读成「多数赢」。

### 10.2 为什么 9T 输：这是 per-tile 内核效率的战场

两边同为 Winograd、同为 NCHW、都 numactl 绑核，转换开销又小（用户实测）——差距全部集中在**核心的 per-tile 工作效率**，不在工程包装层。代码已核实的 4 个结构性原因：

1. **GEMM 单线程 OpenBLAS vs arm_gemm JIT**：我们 `openblas_set_num_threads(1)` + `cblas_sgemm` 钉死单线程小矩阵（`src/winograd_conv.cpp:240-246,331`）；对方就是 arm_gemm JIT。**同一套内核**——`winograd_gemm` 的 arm_gemm 分支（233-239）已写好、默认未开。这是最大、最容易的一步。
2. **变换内核**：编译器生成 vs 对方手写 SVE 汇编（ACL `sve_fp32_6x6.cpp`，完全展开、周期级调度）。
3. **scatter/gather**：Phase 1 散入 U + Phase 3 从 M 收集 = 两笔全量内存遍历；对方 `matrix_stride` 布局让变换直接写进 GEMM 顺序，不做 scatter/gather。
4. **权重变换每调用重算**：对方把 TransformedWeights 缓存进 primitive，跨调用复用。

**关键修正**：本文 §4.1 的「固定开销优势」在 9T 档不成立——9T 是「固定开销已摊薄、带宽未饱和」的甜点区，谁的内核 per-tile 效率高谁赢。且 4T 也是 0/9，说明这些 shape 下固定开销本来就不是胜负手；之前的 8/9 反超主要发生在 **16 线程 + NHWC 微基准 + 单次调用** 场景。

### 10.3 9T 定向优化路径（按性价比）

1. **arm_gemm JIT**（`-DUSE_ARM_GEMM=ON -DARM_GEMM_ROOT=...`）——与对手内核对齐，GEMM 受限 row 直接拉平。CMake 已接好（`CMakeLists.txt:11-66`）。小优化：`GemmHybrid` 每次调用构造一次（36 次/批），可提为静态缓存按 shape 复用。
