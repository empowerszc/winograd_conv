...
### oneDNN 16 线程性能参考（全部 stride=1 group=1 case）

以下数据来自 oneDNN 算子在鲲鹏 920F 上 16 线程的实测结果：

| Case | Input (N,IC,IH,IW) | Weight (OC,IC,3,3) | Count | oneDNN t16(ms) | 本项目 t16(ms) | 差距 |
|------|---------------------|---------------------|-------|---------------|--------------|------|
| 0 | 4,192,40,40 | 192,192 | 24 | 3.55 | 7.0 | 1.97x |
| 1 | 4,96,80,80 | 96,96 | 17 | 4.22 | 5.9 | 1.40x |
| 2 | 4,48,160,160 | 48,48 | 8 | 4.06 | 10.6 | 2.61x |
| 3 | 4,192,20,20 | 192,192 | 16 | 2.83 | 2.3 | 0.81x ✓更快 |
| 4 | 4,384,80,80 | 96,384 | 1 | 13.78 | 8.2 | 0.60x ✓更快 |
| 5 | 4,768,40,40 | 96,768 | 1 | 9.09 | 6.3 | 0.69x ✓更快 |

**关键发现**：
- Case 3/4/5 本项目 **已经快于** oneDNN（0.60x~0.81x）
- Case 0/1/2 仍慢于 oneDNN（1.40x~2.61x）
- Case 4（IC=384, tiles=400）本项目 8.2ms vs oneDNN 13.8ms，**快 40%**
- Case 5（IC=768, tiles=100）本项目 6.3ms vs oneDNN 9.1ms，**快 31%**

Case 3/4/5 快于 oneDNN 的原因：大 IC（384/768）时变换计算量大，OpenMP 并行效果好；oneDNN 的 arm_gemm JIT 对这些矩阵大小可能不如 OpenBLAS。

Case 0/1/2 慢于 oneDNN 的原因：IC 较小（48-192），变换计算量小，OpenMP fork/join + barrier 开销占比大；oneDNN 的 NEScheduler 细粒度调度更高效。
...
| ⑦ +合并权重到主region | 待测 | 待测 | 待测 | 待测 | 待测 | — |
| oneDNN 参考 | 18.0 | 4.9 | 4.0 | 3.6 | — | 1.0x |

**阶段⑥教训**：权重变换放独立 `#pragma omp parallel`，Case 0/1 变慢（额外 fork/join ~1-2ms），Case 3/4/5 改善（-19~-57%）。Case 3 t32: 2.3→1.0ms，Case 5 t32: 5.6→4.0ms。阶段⑦ 合并进主 region 修复回退。
