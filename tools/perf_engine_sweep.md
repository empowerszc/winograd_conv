...
## 终局（2026-08-28，commit bfd6b1e）

散写回归修复后的同作业全量对照：**arm 59/59 全胜 OpenBLAS**，geomean 1.99x、合计 2.50x；
大形状 3.02x、深层 5.3x；修复前输的 9~10 形状全部翻赢。完整 59 行表、修复验证（E3 vs E5）、
oneDNN 对照流程与遗留项见 **`docs/final_benchmark_bfd6b1e.md`**。本节 sweep 仅保留「引擎相对排序」的
参考价值，且当时跑的可能仍是改造前二进制，结论以终局文档为准。
